...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, ActivityIndicator, Modal, Image, Alert, Animated, RefreshControl, ActionSheetIOS, Platform, LayoutAnimation, UIManager, ScrollView } from 'react-native';
...
export default function InvoicesScreen() {
    ...
    useEffect(() => {
        if (filter) {
            ...
            // Reset filters if no param (e.g. Tab click)
            setStatusFilter('all');
            setDateFilter('all');
        }
    }, [filter]);

    // Filter invoices based on status and date
    const filteredInvoices = useMemo(() => {
        let result = invoices;

        // Status Filter
        if (statusFilter === 'paid') result = result.filter(inv => inv.status === 'PAID');
        else if (statusFilter === 'pending') result = result.filter(inv => inv.status !== 'PAID');
        ...
        if (dateFilter === 'today') {
            result = result.filter(inv => {
                // Return true if no due date (treat as immediate/reminder) OR if due today/overdue
                if (!inv.content?.dueDate) return true;
                const d = new Date(inv.content.dueDate);
                d.setHours(0, 0, 0, 0);
                // Include Today AND Overdue (consistent with Home "Reminders")
                return d.getTime() <= today.getTime();
            });
        } else if (dateFilter === 'week') {
            result = result.filter(inv => {
                if (!inv.content?.dueDate) return false;
                const d = new Date(inv.content.dueDate);
            ...
            result = result.filter(inv => {
                if (!inv.content?.dueDate) return false;
                const d = new Date(inv.content.dueDate);
                d.setHours(0, 0, 0, 0); // compare dates
                return d < today;
            });
        }
    ...
    const fetchInvoices = async () => {
        try {
            ...
            Alert.alert('Error', `Failed to load invoices: ${(error as Error).message}`);
        } finally {
            setIsLoading(false);
            setRefreshing(false);
        }
