...
import { useDemoTracking, SignUpPromptDialog, EmailCaptureDialog } from './landing-page-client';
...
const ITEMS = [
  ...
  },
  {
    ...
    actionName: 'viewed_clients_projects',
  },
  {
    title: 'Payroll & Team',
    description:
  ...
  },
  {
    ...
    actionName: 'viewed_integrations',
  },
];

export function FeaturesShowcase() {
  const [active, setActive] = useState(0);
  const { trackDemoStart, trackDemoAction, promptOpen, dismissPrompt, closePrompt, closeEmailFallback, emailFallbackOpen } = useDemoTracking();
  ...
  const handleFeatureClick = (i: number) => {
    setActive(i);
    trackDemoAction(ITEMS[i].actionName);
  };
  ...
  return (
    <>
      <section id="features" className="border-t border-[var(--color-surface-tertiary)] bg-[var(--color-surface)] px-8 py-24">
        <div className="mx-auto max-w-[1400px]">
          <div className="grid gap-16 lg:grid-cols-[400px_1fr] lg:items-start xl:grid-cols-[460px_1fr]">
            ...
            {/* ── Left — timeline list ─────────────────────────── */}
            <div>
              <p className="mb-3 text-[11px] font-bold text-[var(--color-text-muted)]">
                The platform
              </p>
      ...
      {/* Sign-up prompt dialog on high-intent demo action */}
      <SignUpPromptDialog
        open={promptOpen}
        onClose={closePrompt}
      ...
      {/* Email fallback after dismissing sign-up prompt */}
      <EmailCaptureDialog
        open={emailFallbackOpen}
        title="Get product updates"
        submitLabel="Subscribe"
        onClose={closeEmailFallback}
      />
    </>
  );
}
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-[var(--color-surface-secondary)] px-6 py-4">
        <p className="text-[11px] font-bold text-[var(--color-text-muted)]">Payments</p>
      ...
      <div className="flex-1 overflow-hidden px-6 pt-4">
        <div className="overflow-hidden rounded-xl bg-[var(--color-surface)] ring-1 ring-[var(--color-border)]">
          <div className="grid grid-cols-[1fr_auto_auto_auto] gap-x-4 border-b border-[var(--color-surface-secondary)] px-4 py-2.5">
            ...
            <p className="text-[10px] font-bold text-[var(--color-text-muted)]">Amount</p>
            <p className="text-[10px] font-bold text-[var(--color-text-muted)]">Status</p>
          </div>
...
function BookkeepingPanel() {
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-[var(--color-surface-secondary)] px-6 py-4">
        <p className="text-[11px] font-bold text-[var(--color-text-muted)]">Bookkeeping</p>
...
function ClientsPanel() {
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-[var(--color-surface-secondary)] px-6 py-4">
        <p className="text-[11px] font-bold text-[var(--color-text-muted)]">Clients & Projects</p>
...
function PayrollPanel() {
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-[var(--color-surface-secondary)] px-6 py-4">
        <p className="text-[11px] font-bold text-[var(--color-text-muted)]">Payroll & Team</p>
        ...
        <div className="mb-3 grid grid-cols-2 gap-px overflow-hidden rounded-xl bg-[var(--color-border)] ring-1 ring-[var(--color-border)]">
          {[
            ...
            <div key={s.l} className="bg-[var(--color-surface)] px-4 py-3">
              <p className="text-[10px] font-semibold text-[var(--color-text-muted)]">{s.l}</p>
...
function IntegrationsPanel() {
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-[var(--color-surface-secondary)] px-6 py-4">
        <p className="text-[11px] font-bold text-[var(--color-text-muted)]">Integrations</p>
