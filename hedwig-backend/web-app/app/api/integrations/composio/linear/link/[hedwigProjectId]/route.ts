...
type RouteContext = { params: Promise<{ hedwigProjectId: string }> };
...
export async function GET(_req: NextRequest, context: RouteContext): Promise<Response> {
  ...
  const { hedwigProjectId } = await context.params;

  const resp = await fetch(
    `${backendConfig.apiBaseUrl}/api/integrations/composio/linear/link/${encodeURIComponent(hedwigProjectId)}`,
...
export async function DELETE(_req: NextRequest, context: RouteContext): Promise<Response> {
  ...
  const resp = await fetch(
    `${backendConfig.apiBaseUrl}/api/integrations/composio/linear/link/${encodeURIComponent(hedwigProjectId)}`,
    { method: 'DELETE', headers: { Authorization: `Bearer ${session.accessToken}` }, cache: 'no-store' }
  ).catch(() => null);
