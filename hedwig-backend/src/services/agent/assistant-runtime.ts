...
const CHAT_SYSTEM_INSTRUCTION = [
  ...
  'CRITICAL: write tools (e.g. calendar_create_event or docs_create_document) DO NOT execute the action — they stage a suggestion the user must approve. Always call the tool when the user asks for a supported action; never assume you cannot do it.',
  'After staging an action, tell the user which suggestion has been drafted and where to approve it.',
  'Use read tools to gather context before staging writes (e.g. look up client details before drafting an email).',
